...
|-----------------------+---------------------------------------------------------------------------------------------------------------+------------------------------------------------------+------------------------------------------------------|
  ...
  let TableSelection = class {
      ...
      constructor(startCell, endCell, focus) { // startCell, endCell are CellRefs
        ...
        // Don't allow row indices TH's to be part of the selection, so
        // when we find one at the start of the selection, we will bump
        // the start to the nearest TD
        if (this.actualStartCell.node.className == "rowIndex") {
          // Given a node (the TH.rowIndex) we want to find the next sibling TD but we
          // have a node and not an element so we need to traverse back up to the parent
          // and then iterate through the parent's direct children. nextElementSibling
          // would have been nicer as nextSibling can give us something that isn't a TD.
          const parent = this.actualStartCell.node.parentNode;
          const children = Array.from(parent.children);
          const matchingChild = children.find(child => child.tagName.toLowerCase() === 'td');

          this.actualStartCell = CellRef.fromNode(matchingChild)
          this.actualEndCell = (endCell === undefined) ? this.actualStartCell : endCell;
        }
    ...
    for (var table of tables) {
      ...
      var startDrag = function(startEvent) {
        ...
        const handleEntry = function(entryEvent) {
          var eventRange = (entryEvent.target.closest("tr,th,td").matches("thead tr:first-child th[scope='col']"))
