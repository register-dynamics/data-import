window.addEventListener("load", function() {
    ...
    let CellRef = class {
      ...
      static fromNode(node) {
        if(fromNodeCache.has(node)) {
          ...
          console.log("ABS DEBUG fromNode cache hit", node.textContent, cell.row, cell.col);
          return cell;
        } else {
      ...
      static fromCoords(table, row, col) {
        ...
        if (fromCoordsCache.has(table) && fromCoordsCache.get(table).has(cacheKey)) {
          ...
          console.log("ABS DEBUG fromCoords cache hit", row, col, cell.node.textContent);
          return cell;
        }
  ...
  let TableSelection = class {
      ...
      constructor(startCell, endCell, focus) { // startCell, endCell are CellRefs
        ...
        // Iterate through cells, updating min/max if needed
        const cells = this.getRangeOfCells(minRow, maxRow, minCol, maxCol);
        ...
        // Set start/end cells to cover effective range
      ...
      getRangeOfCells(rowStart, rowEnd, colStart, colEnd) {
        ...
        for (var row = rowStart; row <= rowEnd; row++) {
          for (var col = colStart; col <= colEnd; col++) {
            cells.add(CellRef.fromCoords(this.table, row, col));
          }
        }
