window.addEventListener("load", function() {
    ...
    let CellRef = class {
      ...
      static fromNode(node) {
        if(fromNodeCache.has(node)) {
          ...
          console.log("ABS DEBUG fromNode cache hit", node.textContent, cell.col, cell.row);
          return cell;
        } else {
      ...
      static fromCoords(table, row, col) {
        ...
        if (fromCoordsCache.has(table) && fromCoordsCache.get(table).has(cacheKey)) {
          ...
          console.log("ABS DEBUG fromCoords cache hit", col, row, cell.node.textContent);
          return cell;
        }
        ...
        console.log("ABS DEBUG fromCoords", row, col);
        ...
        while(true) {
          ...
          if(rowIdx <= row && colLogicalIdx <= col &&
            ...
            // ABS DEBUG: currentCell.textContent = currentCell.textContent + " (" + col + "," + row + ")";
          ...
          if(colIdx >= currentRow.cells.length - 1 || colLogicalIdx > col) {
            ...
            if(rowIdx >= table.rows.length || rowIdx > row) {
              ...
              console.log("ABS DEBUG fromCoords abort with failure");
              return null;
            }
          ...
          console.log("ABS DEBUG fromCoords loop...");
        }
      }
    }
  ...
  let TableSelection = class {
      ...
      constructor(startCell, endCell, focus) { // startCell, endCell are CellRefs
        ...
        // FIXME: Expand range to cover any spanned cells in the range, and store in startCell/endCell
        ...
        console.log("ABS DEBUG TableSelection start" + minCol + "," + minRow + " -> " + maxCol + "," + maxRow);
        ...
        while (cells.length != 0) {
          ...
          console.log("ABS DEBUG TableSelection loop " + minCol + "," + minRow + " -> " + maxCol + "," + maxRow, cell);
        ...
        console.log("ABS DEBUG TableSelection done " + minCol + "," + minRow + " -> " + maxCol + "," + maxRow);
        ...
        console.log("ABS DEBUG TableSelection cells", this.startCell, this.endCell);
      ...
      getRangeOfCells(rowStart, rowEnd, colStart, colEnd) {
        ...
        console.log("ABS DEBUG getRangeOfCells " + colStart + "," + rowStart + " -> " + colEnd + "," + rowEnd);
        var cells = new Set();
        for (var row = rowStart; row <= rowEnd; row++) {
          for (var col = colStart; col <= colEnd; col++) {
            const candidateCell = CellRef.fromCoords(this.table, row, col)
            cells.add(candidateCell);
            console.log("ABS DEBUG getRangeOfCells cell ", candidateCell);
          }
        }
        console.log("ABS DEBUG getRangeOfCells result", cells);
      ...
      static fromElement(element) {
        // ABS FIXME - does this still work when there's merged cells?
        if (element.nodeName == "TABLE") {
          ...
          var startCell = CellRef.fromNode(element.cells[0]);
          var endCell = CellRef.fromNode(element.cells[element.cells.length - 1]);
