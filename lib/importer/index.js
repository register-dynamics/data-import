...
exports.Initialise = (config, router, prototypeKit) => {
  ...
  prototypeKit.views.addFunction("importerMappedData", (data) => {
    ...
    return { rows: objects, headers: headers };
  });
  ...
  router.post(IMPORTER_ROUTE_MAP.get("importerSelectHeaderPath"), (request, response) => {
    /* selection will be in the following properties of request.body, 
       but may be empty, in which case we should error.

      'importer:selection:TLRow': '1',
      'importer:selection:TLCol': '1',
      'importer:selection:BRRow': '2',
      'importer:selection:BRCol': '2'
  */

      // if (!request.body.sheet || request.body.sheet == "" ) {
      //   request.session.data[IMPORTER_ERROR_KEY] = "Please select the header row"
      //   response.redirect(request.get('Referrer'))
      //   return 
      // }
