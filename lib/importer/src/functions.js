...
// a sheet.
//--------------------------------------------------------------------
const importTablePreview = (data, sheet) => {
    const session_data = data[IMPORTER_SESSION_KEY];
    const session_obj = new session_lib.Session(session_data)
    const sheet_tables = sheets_lib.ListTables(session_obj.backendSid, sheet)

    return sheet_tables.map((tableName) => {
        return {
            name: tableName,
            data: { rows: sheets_lib.GetTablePreview(session_obj.backendSid, sheet, tableName) },
        };
    });
}
