...
// to
// [
//  { message: "An error occurred", fields: [
//      { field: "A", rows: [1] },
//      { field: "B", rows: [1, 2] }
//    ]
//  }
// ]
// This groups results first by error message, then by field, collecting all row indices for each field.
const remapErrorStructure = (errors, row_data) => {
    ...
    for (const error of errors) {
        ...
        const fieldMap = messageMap.get(error.message);

        if (!fieldMap.has(error.field)) {
            fieldMap.set(error.field, new Set());
        }
        const rowsSet = fieldMap.get(error.field);
        ...
        if (row_data && Array.isArray(row_data) && error.row < row_data.length)  {
            ...
            if (row_data && Array.isArray(row_data)) {
                if (error.row > 0) {
                    rowsSet.add(row_data[error.row - 1]);
                }
    ...
    return Array.from(messageMap.entries()).map(([message, fieldMap]) => ({
        message,
        fields: Array.from(fieldMap.entries()).map(([field, rowsSet]) => ({
            field,
