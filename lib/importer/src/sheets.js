...
exports.GetTotalColumns = (session, sheet) => {
  let s = sheet ? sheet : session.sheet;

  return backend
    .SessionGetInputDimensions(session.backendSid)
    .sheetDimensions.get(s)
    .columns;
};

// Returns the total number of rows from the input either the named
// sheet or the currently selected sheet
exports.GetTotalRows = (session, sheet) => {
  let s = sheet ? sheet : session.sheet;
...
exports.MapData = (session, previewLimit = DEFAULT_PREVIEW_LIMIT) => {
  ...
  for (const [columnIndex, attributeName] of Object.entries(session.mapping)) {
    ...
    const f = session.fields.find((x) => x.name == attributeName)
    if (f) {
