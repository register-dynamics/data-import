...
function parseYear(x) {
  ...
  if (xx< 100) {
    ...
    if(xx>=80) { // Let's make a guess
      return [[],['Guessing a two-digit year '+xx+' is in the 20th century'],1900+xx];
    } else {
      return [[],['Guessing a two-digit year '+xx+' is in the 21st century'],2000+xx];
    }
  } else {
    return [[],[],xx];
  }
...
function parseMonth(x) {
  let nameGuess = monthNames[x.toLowerCase()]
  if(nameGuess) {
    return [[],[],nameGuess];
  }
  ...
  let xx = Number(x);
  if(!Number.isInteger(xx)) {
    return [["Months must either be numbers or recognised names, but we found "+x],[],undefined];
  }

  if(xx < 1 || xx>12) {
    return [["Month numbers must be in the range 1-12, but we found "+x],[],undefined];
  }
...
function parseDay(x, year, month) {
  // Remove any ordinal suffixes - FIXME extend this with other languages and conventions?
  if (x.endsWith('st') || x.endsWith('th') || x.endsWith('nd') || x.endsWith('rd')) {
    x = x.slice(0, x.length-2);
  }
  let xx = Number(x);

  if (Number.isNaN(xx)) {
    return [["Day must be a number, but we found "+x], [], undefined];
  }

  let maxDay = daysInMonth(year, month);
  if(xx<1 || xx>maxDay) {
    return [["Day numbers must be in the range 1-" + maxDay + " for this month, but we found "+xx],[],undefined];
  }
...
exports.makeCombinedDateType = (format) => {
  ...
  return (inputCell) => {
    ...
    // Is the input value a string... or something else, that we'll turn into a string and do our best with?
      ...
      if(yearIndex == -1) {
        return ati.failedMapping([],["A written-out date was found, but we don't know a date format"]);
      }
