...
const IMPORTER_ROUTE_MAP = new Map([
  ...
  ["importerSelectTablePath", "/importer/tables"],
  ...
  ["importerDevelopment", "/importer/dev"]
]);
...
exports.Initialise = (config, router, prototypeKit) => {
  ...
  router.post(
    ...
    (request, response) => {
      const session_data = request.session.data[IMPORTER_SESSION_KEY];
      if (!session_data) {
        response.status(404);
        return;
      }
      ...
      let check = sheets_lib.GetPreview(session.backendSid, session.sheet);
      if (check == null) {
        request.session.data[IMPORTER_ERROR_KEY] =
      ...
      let maxCol = sheets_lib.GetTotalColumns(session.backendSid, session.sheet);
      ...
      let tableList = sheets_lib.ListTables(session.backendSid, session.sheet)
  ...
  //--------------------------------------------------------------------
  // When the user has chosen a table to use, this route stores the table
  // name in the session for later steps.  If no table name is provided
  // then no table has been selected.
  //--------------------------------------------------------------------
  router.post(
    IMPORTER_ROUTE_MAP.get("importerSelectTablePath"),
    (request, response) => {
      const session_data = request.session.data[IMPORTER_SESSION_KEY];
      if (!session_data) {
        response.status(404);
      ...
      if (!request.body.table || request.body.table == "") {
        request.session.data[IMPORTER_ERROR_KEY] =
      ...
      if (request.body.table == "no_table") {
        ...
        let maxCol = sheets_lib.GetTotalColumns(session.backendSid, session.sheet);
        let maxRow = sheets_lib.GetTotalRows(session.backendSid, session.sheet)
        ...
        let headerRange = sheets_lib.GetTableHeader(session.backendSid, session.sheet, session.table)
        ...
        let footerRange = sheets_lib.GetTableFooter(session.backendSid, session.sheet, session.table)
        let rowNum = sheets_lib.GetTableRows(session.backendSid, session.sheet, session.table)
        footerRange = {
          start: { row: Math.min(rowNum, 10) + 1, column: footerRange.start.column },
          end: { row: Math.min(rowNum, 10) + 1, column: footerRange.end.column } //need to set this as default, so footer is rome immediately after table end when selecting againsty last 10 rows
        }
  ...
  router.post(
    ...
    (request, response) => {
      let session_data = request.session.data[IMPORTER_SESSION_KEY];
      if (!session_data) {
        response.status(404);
        return;
      }
  ...
  router.post(
    ...
    (request, response) => {
      ...
      // Ensure we add the header range to the selection range otherwise it will not be accurate
      if (session.headerRange) {
        const offset = 1 + session.headerRange.end.row - session.headerRange.start.row;
        selectionRange.start.row += offset;
        selectionRange.end.row += offset;
      }
  ...
  if (developmentMode) {
    ...
    router.get(
      IMPORTER_ROUTE_MAP.get("importerDevelopment"),
      (request, response) => {
        response.status(200)
...
const getSelectionFromRequest = (request, session, optional = false) => {
  ...
  let defaultMaxCol = optional ? -1 : (sheets_lib.GetTotalColumns(session.backendSid, session.sheet) - 1);
