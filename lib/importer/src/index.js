...
exports.Initialise = (config, router, prototypeKit) => {
  ...
  //     "All {n} rows of {sheet}"
  //--------------------------------------------------------------------
  prototypeKit.views.addFunction("importerGetTableCaption", (data, prefix, rowsAskedFor, sheetName=null) => {
    const session = data[IMPORTER_SESSION_KEY];
...
const getSelectionFromRequest = (request, session, optional=false) => {
  ...
  let brCol = getIntOrDefault(request.body['importer:selection:BRCol'], defaultMaxCol);
