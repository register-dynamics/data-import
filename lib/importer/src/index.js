...
exports.Initialise = (config, router, prototypeKit) => {
  ...
  router.post(
    ...
    (request, response) => {
      ...
      let check = sheets_lib.GetPreview(session.backendSid, request.body.sheet);
      if (check == null) {
        request.session.data[IMPORTER_ERROR_KEY] =
  ...
  router.get('/importer/debug/valid', function (request, response) {
    const sess = debug_lib.CreateSessionWithFileFixture(plugin_config, request, "validation-test.csv")
    sess.sheet = "Sheet1"
    sess.headerRange = {
      sheet: "Sheet1",
    ...
    const data = {}
    data[IMPORTER_SESSION_KEY] = sess
    const res = funcs_lib.importerMappedData(data)

    response.render("debug/valid.html", res)
  })
