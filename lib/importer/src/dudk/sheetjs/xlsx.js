...
function parse_table_xml(data, opts) {

  var tabletag = str_match_xml_ns(data, "table");

  var tm = tabletag[0].match(/<(?:\w+:)?table[^<>]*>/);

  var tableattrs = parsexmltag(tm[0]);

  var name = tableattrs["name"];
  var displayName = tableattrs["displayName"];
  var ref = tableattrs["ref"];

  var columns = [];

  var clmntag = str_match_xml_ns(data, "tableColumns");
  if(clmntag && clmntag[1]) clmntag[1].split(/<\w*:?tableColumn/).forEach(function(x) {
    if (x === "" || x.trim() === "") return;
    
    var colAttrs = parsexmltag(x);
    columns.push({name: colAttrs["name"],
                  id: colAttrs["id"]});
  });
...
function safe_parse_sheet(zip, path, relsPath, sheet, idx, sheetRels, sheets, stype, opts, wb, themes, styles) {
	try {
		...
		/* scan rels for comments and threaded comments and tables */
        ...
        if (sheetRels[sheet][n].Type == RELS.TABLE) {
          dfile = resolve_path(sheetRels[sheet][n].Target, path);
          if(!_ws['!tables']) _ws['!tables'] = [];
          // FIXME: Parse the table file
          _ws['!tables'].push(parse_table_xml(getzipdata(zip, dfile, true), opts));
        }
		});
