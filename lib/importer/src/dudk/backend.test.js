...
test('validation', () => {
  ...
  for (const [filename, sheetname] of fixtures) {
    ...
    expect(warnings).toMatchObject(new Map([
      [4, [{ "row": 4, "message": 'Row is empty' }]],
...
test('guessing types', () => {
  ...
  expect(guesses).toMatchObject([
    // Order is actually irrelevant for all types lists
    {full:true, types:["number","text"]},
    {full:true, types:["date(ymd)","text"]},
    {full:true, types:["date(ydm)","text"]},
    {full:true, types:["date(dmy)","text"]},
    {full:true, types:["date(mdy)","text"]},
    {full:true, types:["date(ymd)","date(ydm)","text"]},
    {full:false, types:["number","text"]},
    {full:true, types:["text"]},
    {full:false, types:["text"]}
  ]);
});
