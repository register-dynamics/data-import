...
test('validation', () => {
  ...
  for (const [filename, sheetname] of fixtures) {
    ...
    expect(warnings).toStrictEqual([
      { "row": 4, "message": 'Row is empty' },
      { "row": 8, "field": "Weight", "message": "This is not a valid number" }]);

    const errors = backend.JobGetErrors(sid, jid);
    expect(errors).toStrictEqual([
      { "row": 6, "field": 'Age', "message": "This is not a valid number" },
      { "row": 7, "field": 'Name', "message": "'Name' must be supplied" },
