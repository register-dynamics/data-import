...
test('validation', () => {
  ...
  for (const [filename, sheetname] of fixtures) {
    ...
    expect(warnings).toStrictEqual([
      ...
      { "row": 7, "field": "Weight", "message": "This is not a valid number" }]);

    const errors = backend.JobGetErrors(sid, jid);
    expect(errors).toStrictEqual([
      { "row": 5, "field": 'Age', "message": 'This is not a valid number' },
      { "row": 6, "field": 'Name', "message": 'A value must be provided' },
