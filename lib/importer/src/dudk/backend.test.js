...
describe("Backend tests", () => {
  ...
  test('happy path', () => {
    ...
    for (let [filename, sheetname] of fixtures) {
      ...
      const mapping = {
        ...
        attributeTypes: {
          Name: attributeTypes.requiredType(attributeTypes.basicStringType, {name: 'Name'}),
          Age: attributeTypes.requiredType(attributeTypes.basicNumberType, {name: 'Age'}),
          Egginess: attributeTypes.requiredType(attributeTypes.basicStringType, {name: 'Egginess'})
        },
      };
      ...
      // Temporarily disabling warnings as warnings are not expected, but
      // are being returned here for row length in the test.* fixtures.
  ...
  test('tables', () => {
    const sid = backend.CreateSession();
    backend.SessionSetFile(sid, '../../fixtures/tables_test.xlsx');

    const tables = backend.SessionGetInputTables(sid);
    expect(tables).toStrictEqual(new Map([
      ["Standard",
       {
         tableCount: 3,
         tableNames: ['Scoundrel','Scout','Soldier']
       }],
      ["Jedi",
       {
         tableCount: 3,
         tableNames: ["Sentinel","Guardian","Consular"]
       }]
    ]));

    const tableRanges = backend.SessionGetInputTableRanges(sid,"Standard");
    console.log(tableRanges)
    expect(tableRanges).toStrictEqual(new Map([
      ["Soldier", {
        displayName: "Soldier",
        range: {
          start: {
  ...
  for (const [filename, sheetname] of fixtures) {
    ...
    const mapping = {
      ...
      attributeTypes: {
        Name: attributeTypes.requiredType(attributeTypes.basicStringType, {name: 'Name'}),
        Age: attributeTypes.requiredType(attributeTypes.basicNumberType, {name: 'Age'}),
        Egginess: attributeTypes.optionalType(attributeTypes.basicStringType, {name: 'Egginess'}),
        Weight: attributeTypes.optionalType(attributeTypes.basicNumberType, {name: 'Weight'})
      }
    };
    ...
    expect(warnings).toMatchObject([
      { "row": 4, "type": {variant: "EmptyRow"} },
      { "row": 8, "field": "Weight", "type": {variant: "IncorrectType"} }]);

    const errors = backend.JobGetErrors(sid, jid);
    expect(errors).toMatchObject([
      { "row": 6, "field": 'Age', "type": { variant: "IncorrectType"} },
      { "row": 7, "field": 'Name', "type": { variant: "FieldRequired"} },
      { "row": 7, "field": 'Age', "type": { variant: "IncorrectType"} }
    ]);
