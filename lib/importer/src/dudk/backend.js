  ...
  for (let rowIdx = range.start.row; rowIdx <= range.end.row; rowIdx++) {
    ...
    if (row) {
      if (row.length < expectedColumnCount) {
        warnings.push({index: rowIdx, row: row, message: "Row has fewer columns than expected, wanted " + expectedColumnCount + " but found " + row.length})
      }
    ...
    if (foundSomeValues) {
      ...
      Object.entries(recordCells).forEach((element) => {
        ...
        result.warnings.forEach((text) => {
          warnings.push({ row: rowIdx - 1, field: attr, message: text });
        });
        ...
        if (result.valid) {
          ...
          result.errors.forEach((text) => {
            rowErrorCount += 1;
            if (includeErrorRow) {
              errors.push({ row: rowIdx - 1, field: attr, message: text, data: row });
            } else {
              errors.push({ row: rowIdx - 1, field: attr, message: text });
            }
      ...
      warnings.push({ row: rowIdx, message: "Row is empty" })
    }
  }
