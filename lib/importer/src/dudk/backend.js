...
exports.SessionSetFile = (sid, filename) => {
  ...
  //wbx.xlsx.load(ab);
  if (filename.match(/^.*tables_test.xlsx$/)) {
    console.log("DEBUG workbook", wb);
  }
...
//Set filename for exceljs import session.
/*
exports.SessionSetxlsxFile = (sid, filename) => {
  const ab = filename.arrayBuffer();
  ...
  let sessionx = {
    filename: filename,
    wb: wbx,
    sheetNames: Array.from(Object.keys(wbx.worksheets)),
    headerRanges: {}, // A map from sheetname to the header range
    footerRanges: {}, // A map from sheetname to the footer range
    mappingRules: {},
    jobs: new Map(),
...
function getTables(sid) {
  ...
  console.log("DEBUG workbook", session.wbx);
  Object.keys(session.wb.Sheets).forEach((sheetName) => {
    const sheetx = session.wbx.getWorksheet(sheetName);

    sheetTables.set(sheetName, {
      tableCount: Object.keys(sheetx.tables).length,
      tableNames: Object.keys(sheetx.tables)
    })
  })
...
function getTableRanges(sid, sheetName) {
  ...
  Object.keys(session.wbx.getWorksheet(sheetName).tables).forEach((key) => {
    const table = session.wbx.getWorksheet(sheetName).tables[key].table;
    const name = table.name;

    if (table["tableRef"]) {
      const range = xlsx.utils.decode_range(table["tableRef"]);
      const colnames = []
      Object.keys(table.columns).foreach((id) => {
        colnames.push(table.columns[id].name);
      })

      tableRanges.set(name, {
        ref: table["tableRef"],
        range: range,
        colnames: colnames
      })
  ...
  tableRangesCache.set(sid, { tableRanges: tableRanges })
