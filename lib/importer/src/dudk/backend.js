...
// TODO: Move this into a `frontend` package along with everything that is useful to the client
// but for now this is a placeholder for the default error messages unless the user
// overwrites them with a call to SetErrorMessages
let error_messages = require('path').join(__dirname, '../../resources/error_messages.json');
...
// Sets the error messages for any subsequent sessions created.
// If this is not called by the client, then the default set
// is used.
exports.SetErrorMessages = (messages_file) => {
  error_messages = errorTypes.ErrorMessages.load(messages_file)
};
