...
exports.SessionSetHeaderRange = (sid, range) => {
  ...
  let session = sessionStore.get(sid);
  session.headerRanges[range.sheet] = range;
  sessionStore.set(sid, session);
};
...
exports.SessionSetFooterRange = (sid, range) => {
  let session = sessionStore.get(sid);
  session.footerRanges[range.sheet] = range
  sessionStore.set(sid, session);
};
...
// interpreted as extra text values. The return value is an array, indexed on
// the relative column number within the range; each element is an object with
// elements "types" listing a range of possible types and "full" indicating
// if the column has no missing values.
exports.SessionGuessTypes = (sid, range) => {
  ...
  if (range.end.row < range.start.row) {
    ...
    guesses.fill({types: [], full: false});
    return guesses;
  }

  // Extract list of all type names
  let typeNames = guesser.TYPE_PATTERNS.map((pattern) => pattern.name);
  typeNames.push("text"); // The text type is special and hardcoded, rather than
                          // having a regexp that matches anything

  // Initialise state, that we will refine as we go
  for (let col=0;col<columns;col++) {
    guesses[col] = {types: typeNames.slice(), // Could be any type so far. This
                                              // makes a clone of the array so
                                              // we can mutate it later.
