...
exports.ListTables = (sid, sheet) => {
  ...
  const tables = Array.from(tableRanges.tableRanges.keys());
  return tables
}

exports.GetTableRange = (sid, sheet, table) => {
  const tableRanges = backend.SessionGetInputTableRanges(sid, sheet);
  const tableData = tableRanges[table]
  const range = tableData['range']
  return range
}
...
exports.GetTablePreview = (sid, sheet, table, count=10) => {
  const tableRange = backend.SessionGetInputTableRanges(sid, sheet).tableRanges.get(table).get('range')
  const tablePreview = backend.SessionGetInputSampleRows(sid, {
    sheet: sheet,
    start: tableRange.get('start'),
    end: tableRange.get('End')
  }, Math.min(count, dimensions.rows), 0, 0)[0];
