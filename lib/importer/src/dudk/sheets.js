...
const remapErrorStructure = (errors, row_data, mappingIndices, headerRangeOffset=1) => {
    ...
    return Array.from(messageMap.entries().map(([message, errorObj]) => {
        ...
        const errorRows = Array.from(errorObj.rows.values()).filter(rowObj => rowObj.row.some(element=>element && element.error)).map(rowObj => {rowObj.error = true; return rowObj});
