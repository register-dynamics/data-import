...
exports.GetPreview = (sid, sheet, count = 10) => {
  ...
  const previewRange = new Range(
    sheet,
    { row: 0, column: 0 },
    { row: dimensions.rows, column: dimensions.columns > 0 ? dimensions.columns - 1 : 0 }
  );
  const preview = backend.SessionGetInputSampleRows(sid, previewRange, Math.min(count, dimensions.rows), 0, 0)[0];
...
exports.GetRows = (sid, sheet, start = 0, count = 10) => {
  ...
  const rowRange = new Range(
    sheet,
    { row: start, column: 0 },
    { row: Math.min(start + count, sheetDimensions.rows), column: sheetDimensions.columns - 1 }
  );
...
exports.GetRowRangeFromEnd = (sid, sheet, count = 10) => {
  ...
  return new Range(
    sheet,
    { row: Math.max(end - count, hRange.start.row + 1), column: hRange.start.column },
    { row: end, column: hRange.end.column }
  );
}
...
exports.GetColumnValues = (sid, sheet, columnIndex, cellWidth = 30, count = 10) => {
  ...
  // Create a new Range for just the column we want
  const colStart = dataRange.start.row;
  const colEnd = dataRange.end.row > dataRange.start.row ? dataRange.end.row - 1 : dataRange.end.row;
  const singleColRange = new Range(
    dataRange.sheet,
    { row: colStart, column: dataRange.start.column + columnIndex },
    { row: colEnd, column: dataRange.start.column + columnIndex }
  );
