...
function localFileMock(file, data) {
    ...
    if (process.env.DUDK_SESSION_STORE_DISK) {
        const folder = path.join(process.env.DUDK_SESSION_STORE_DISK, "data-importer-session-store")
        obj[folder] = mock_files.load(folder, { recursive: false, lazy: false })
    }
