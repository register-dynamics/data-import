...
describe("Validation tests", () => {
    ...
    test('Missing required values', () => {
        ...
        withCleanup(() => {
            ...
            expect(row1Errors[0].message).toBe("'Salary' must be supplied")

            expect(row1Errors[1].field).toBe("Contribution percentage")
            expect(row1Errors[1].message).toBe("'Contribution percentage' must be supplied")
        }, () => {
            mock_files.restore();
        });
    });
    ...
    test('Structural warnings', () => {
        ...
        withCleanup(() => {
            ...
            expect(row2Warnings[0]).toEqual({ "row": 2, "message": "Row is empty" })
        }, () => {
            mock_files.restore();
        });
    });
