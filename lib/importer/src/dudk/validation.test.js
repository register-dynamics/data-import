...
function localFileMock(file, data) {
    const obj = {}
    obj[file] = data

    const sessionFolder = process.env.DUDK_SESSION_STORE_DISK
    if (sessionFolder) {
        obj[sessionFolder] = mock_files.load(sessionFolder, { recursive: false, lazy: false })
    }

    mock_files(obj)
}

function arrayItemForRow(array, row) {
    return array.filter((item) => item.row === row);
}
...
describe("Validation tests", () => {
    ...
    test('Validation happy path', () => {
        ...
        withCleanup(() => {
            ...
            localFileMock('./empty/app/config.json', JSON.stringify(config))
            ...
            const jobSummary = backend.JobGetSummary(sess.id, jid);
            ...
            const warnings = backend.JobGetWarnings(sess.id, jid);
            expect(warnings).toMatchObject({});

            const errors = backend.JobGetErrors(sess.id, jid);
    ...
    test('Missing required values', () => {
        ...
        withCleanup(() => {
            ...
            const jid = backend.SessionPerformMappingJob(sess.backendSid, dataRange, mapping);

            const jobSummary = backend.JobGetSummary(sess.backendSid, jid);
            expect(jobSummary).toMatchObject({
                recordCount: 2,
                errorCount: 2,
                warningCount: 0
            ...
            const errors = backend.JobGetErrors(sess.backendSid, jid);
            expect(errors).not.toBe(null)

            const row1Errors = arrayItemForRow(errors, 1)
            expect(row1Errors).toMatchObject([
                {row: 1, field: "Salary", type: {type: "text", value: undefined, variant: "FieldRequired"}},
                {row: 1, field: "Contribution percentage", type: {type: "text", value: undefined, variant: "FieldRequired"}}
            ]);
    ...
    test('Incorrect field type', () => {
        ...
        withCleanup(() => {
            ...
            const row1Warnings = arrayItemForRow(warnings,1)
            expect(row1Warnings[0].field).toBe("Title")
            expect(row1Warnings[0].type).toMatchObject({ variant: "IncorrectType", type: "number", value: "Mr" })
        }, () => {
            mock_files.restore();
        });
    });
    ...
    test('Structural warnings', () => {
        ...
        withCleanup(() => {
            ...
            const warn = backend.JobGetWarnings(sess.id, jid);
            expect(warn).not.toBe(null)

            const row2Warnings = arrayItemForRow(warn, 2)

            expect(row2Warnings).not.toBe(null)
            expect(row2Warnings[0]).toMatchObject({ "row": 2, "type": { variant: "EmptyRow" } })
        }, () => {
            mock_files.restore();
        });
    });
