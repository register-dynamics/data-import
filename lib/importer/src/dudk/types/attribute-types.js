...
exports.requiredType = (baseType, field) => {
  return (inputValue) => {
    if (inputValue && (inputValue.v !== undefined && inputValue.v != "")) {
      ...
      return ati.failedMapping([], [errors.ValidationError.FieldRequired({value: inputValue?.v, type: field.type})]);
    }
  };
}
...
exports.basicNumberType = (inputCell) => {
  ...
  if (isNaN(result)) {
    return ati.failedMapping([], [errors.ValidationError.IncorrectType({ value: inputCell.v, type: "number"})]);
  } else {
