...
test('dates in ods', () => {
  ...
  expect(warnings).toStrictEqual([
    { field: "Date", row: 1, message: "Guessing a two-digit year 31 is in the 21st century" }
  ]);

  const errors = backend.JobGetErrors(sid, jid);
  expect(errors).toStrictEqual([
    { field: "Date", row: 1, message: "Day numbers must be in the range 1-30 for this month, but we found 2025" }
  ]);
...
test('dates in csv', () => {
  ...
  expect(warnings).toStrictEqual([
    { field: "Date", row: 1, message: "Guessing a two-digit year 3 is in the 21st century" },
    { field: "Date", row: 5, message: "Guessing a two-digit year 25 is in the 21st century" },
    { field: "Date", row: 7, message: "Guessing a two-digit year 21 is in the 21st century" }
  ]);
  expect(errors).toStrictEqual([
    { field: "Date", row: 1, message: "Day numbers must be in the range 1-31 for this month, but we found 2025" },
    { field: "Date", row: 2, message: "Unrecognisable date format (expected 3 components, found 4)" },
    { field: "Date", row: 7, message: "Day numbers must be in the range 1-31 for this month, but we found 2025" }
  ]);
