...
test('dates in ods', () => {
  ...
  expect(warnings).toStrictEqual(new Map([
    [2, [{ field: "Date", row: 2, message: "Guessing a two-digit year 31 is in the 21st century" }]]
  ]));

  const errors = backend.JobGetErrors(jid);
  expect(errors).toStrictEqual(new Map([
    [2, [{ field: "Date", row: 2, message: "Day numbers must be in the range 1-30 for this month, but we found 2025" }]]
  ]));
...
test('dates in csv', () => {
  ...
  expect(warnings).toStrictEqual(new Map([
    [2, [{ field: "Date", row: 2, message: "Guessing a two-digit year 3 is in the 21st century" }]],
    [6, [{ field: "Date", row: 6, message: "Guessing a two-digit year 25 is in the 21st century" }]],
    [8, [{ field: "Date", row: 8, message: "Guessing a two-digit year 21 is in the 21st century" }]]
  ]));
  expect(errors).toStrictEqual(new Map([
    [2, [{ field: "Date", row: 2, message: "Day numbers must be in the range 1-31 for this month, but we found 2025" }]],
    [3, [{ field: "Date", row: 3, message: "Unrecognisable date format (expected 3 components, found 4)" }]],
    [8, [{ field: "Date", row: 8, message: "Day numbers must be in the range 1-31 for this month, but we found 2025" }]]
  ]));

  const allRecords = backend.JobGetRecords(jid, 0, 6);
  expect(allRecords).toMatchObject([
...
test('dates in xlsx from numbers', () => {
  ...
  const allRecords = backend.JobGetRecords(jid, 0, 7);
  expect(allRecords).toMatchObject([
...
test('dates in xlsx from office 2016', () => {
  ...
  const allRecords = backend.JobGetRecords(jid, 0, 2);
  expect(allRecords).toMatchObject([
    { Date: { year: 2025, month: 4, day: 4 } },
