...
test('validation', () => {
  ...
  for (const [filename, sheetname] of fixtures) {
    ...
    const jid = backend.SessionPerformMappingJob(sid, dataRange, mapping, true);
    ...
    expect(warnings).toMatchObject(new Map([[4, ['Row is empty']]]));

    const errors = backend.JobGetErrors(jid);
    expect(errors).toMatchObject(new Map([
      [6, [{ "row": 6, "field": 'Age', "message": 'This is not a valid number' }]],
      [7, [{ "row": 7, "field": 'Age', "message": 'This is not a valid number' }]],
