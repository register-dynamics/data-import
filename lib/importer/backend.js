...
exports.SessionSetFile = (sid, filename) => {
    sessionStore[sid].filename = filename;
    sessionStire[sid].data = xlsx.parse(filename);
    sessionStore[sid].jobs = new Set();
};

// Returns the input file structure, as an object with a property "sheets"
// mapping sheet names to objects with "rows" and "columns" properties.
exports.SessionGetInputStructure = (sid) => {
    assert(sessionStore[sid]);
    let data = sessionStore[sid].data;
    let sheetStructure = new Map();
    data.forEach((sheet) => {
        structure[sheet.name] = {
            rows: sheet.data.length,
            columns: sheet.data.reduce(
                (maxLength, currentRow) => max(maxLength, currentRow.length),
                0)
        };
    });
    return {
...
// Returns a sample of rows in a range. range is of the form {start:{row: X, column: Y}, end:{row: X, column: Y}}.
...
exports.SessionGetInputSampleRows = (sid, range, startCount, middleCount, endCount) => {
    ...
    let startRows = data.slice(range.start.row, range.end.row+startCount);
    ...
    while(middleInexes.size < middleCount) {
    ...
    return [startRows.slice(range.start.column, range.end.column+1),
            middleRows.slice(range.start.column, range.end.column+1),
            endRows.slice(range.start.column, range.end.column+1)];
};
...
exports.SessionPerformMappingJob = (sid, range, mapping) => {
    ...
    let job = {
        records: [], // Array of output records
        errors: new Map(), // Maps input row number to list of errors
        warnings: new Map() // Maps input row number to list of warnings
    };
    let data = sessionStore[sid].data;
    let attrMap = mapping.attributeMappings;

    for(let rowIdx=range.start.row; row <= range.end.row; row++) {
        let row = data[rowIdx];
        let record = new Map();
        attrMap.forEach((attr, m) => {
...
exports.JobGetSampleRecords = (jid, startCount, middleCount, endCount) => {
    ...
    let startRecords = records.slice(0, startCount+1);

    // Extract random sample of middle records
    let middleStart = startCount; // First record eligible for middle sample
    let middleEnd = records.size - endCount; // Last record eligible for middle sample
    ...
    let endRecords = records.slice(records.size-endCount, records.size);
