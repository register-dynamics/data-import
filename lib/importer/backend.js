...
function cellToSampleIgnoreMerges(c) {
    if(c) {
        return c.w;
    } else {
        return undefined;
...
// Convert a sheet.js cell object to a string for display in an input sample; but if it's a merged cell, return a small JS object containing merge metadata
function cellToSample(c) {
    if(c) {
        if(c.merge) {
            return {
                merge: c.merge,
                value: cellToSampleIgnoreMerges(c)
            };
        } else {
            return cellToSampleIgnoreMerges(c);
        }
    } else {
        return undefined;
    }
...
exports.SessionGetInputValues = (sid, range, maxValues) => {
    ...
    for(let row = range.start.row; row <= range.end.row; row++) {
        ...
        for(let col = range.start.column; col <= range.end.column; col++) {
            ...
            let value = cellToSampleIgnoreMerges(rowData[col]);
    ...
    for(let col = range.start.column; col <= range.end.column; col++) {
        ...
        result[resultIdx++] = { inputValues: Array.from(columnValues.get(col).values()).sort(),
