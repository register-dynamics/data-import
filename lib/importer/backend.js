...
function clampCounts(startCount, middleCount, endCount, totalRowsInRange) {
    ...
    if ((startCount + middleCount + endCount) > totalRowsInRange) {
        ...
        if(middleCount < 0) {
            ...
            if(endCount == 0) {
                // We didn't want any end rows, so put them all at the start
                startCount = totalRowsInRange;
            } else if(startCount == 0) {
                // We didn't want any start rows, so put them all at the end
                endCount = totalRowsInRange;
            } else {
                // We wanted some of both, so split them evenly
